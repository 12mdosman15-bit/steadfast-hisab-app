...
@app.patch("/api/orders/<int:order_id>/cod")
def edit_order_cod(order_id):
    if (x := require_login()): return x
    d = request.get_json(silent=True) or {}
    try: amount = float(d.get("cod_amount"))
    except (TypeError, ValueError): return jsonify({"ok": False, "message": "সঠিক COD Amount দিন।"}), 400
    if amount < 0: return jsonify({"ok": False, "message": "COD negative হতে পারবে না।"}), 400
    db = SessionLocal()
    try:
        o = db.query(Order).filter_by(id=order_id, user_id=current_user_id()).first()
        if not o: return jsonify({"ok": False, "message": "Order not found."}), 404
        o.cod_amount = amount
        db.commit(); db.refresh(o)
        warning = " Courier-এ আগে পাঠানো হলে Steadfast-এর COD automatic update হবে না; এটি শুধু app-এর হিসাব update করেছে." if o.status not in ("saved", "pending") else ""
        return jsonify({"ok": True, "order": order_dict(o), "message": "COD update হয়েছে।" + warning})
    except Exception as e:
        db.rollback(); return jsonify({"ok": False, "message": "COD update failed.", "details": str(e)}), 500
    finally: db.close()
...
def list_orders():
    ...
    try:
        ...
        status = clean(request.args.get("status")); search = clean(request.args.get("search"))
        if status: q = q.filter(Order.status == status)
        if search:
            from sqlalchemy import or_
            like = f"%{search}%"
            q = q.filter(or_(Order.phone.ilike(like), Order.invoice.ilike(like), Order.consignment_id.ilike(like), Order.tracking_code.ilike(like), Order.customer_name.ilike(like)))
        rows = q.limit(2000).all()
        return jsonify({"ok": True, "orders": [order_dict(x) for x in rows], "count": len(rows)})
    finally: db.close()
...
def expenses():
    ...
    db = SessionLocal()
    try:
        q = db.query(Expense).filter(Expense.user_id == current_user_id()).order_by(Expense.id.desc())
        search = clean(request.args.get("search"))
        if search:
            from sqlalchemy import or_
            like = f"%{search}%"
            q = q.filter(or_(Expense.title.ilike(like), Expense.note.ilike(like)))
        rows = q.limit(2000).all()
        return jsonify({"ok": True, "expenses":[{"id":x.id,"title":x.title,"amount":x.amount,"note":x.note or "","created_at":x.created_at.isoformat()} for x in rows], "count": len(rows)})
    finally: db.close()
...
def supplier_payments():
    ...
    db = SessionLocal()
    try:
        q = db.query(SupplierPayment).filter(SupplierPayment.user_id == current_user_id()).order_by(SupplierPayment.id.desc())
        search = clean(request.args.get("search"))
        if search:
            from sqlalchemy import or_
            like = f"%{search}%"
            q = q.filter(or_(SupplierPayment.supplier_name.ilike(like), SupplierPayment.note.ilike(like)))
        rows = q.limit(2000).all()
        return jsonify({"ok": True, "payments":[{"id":x.id,"supplier_name":x.supplier_name,"amount":x.amount,"note":x.note or "","created_at":x.created_at.isoformat()} for x in rows], "count": len(rows)})
    finally: db.close()
